...
- **`Impiccato` module** — Italian word list (~100 words, no accented characters), 21-letter Italian alphabet keyboard (`ABCDEFGHILMNOPQRSTUVZ`, no J/K/W/X/Y), 9-error limit. The SVG starts with only the ground bar; each error reveals the next `.part[data-part="0..8"]` in order: vertical post, top bar, rope, head, body, left arm, right arm, left leg, right leg. Listens for physical keyboard input but ignores it when the impiccato view is hidden.
